...
export default function FilterPostsReact({ posts = [], categories = [], tags = [] }: FilterPostsReactProps) {
    ...
    if (activeFilter !== 'all') {
        filtered = posts.filter(
            p => (p.category || '').toLowerCase() === filterLower || (p.tags || []).map(t => t.toLowerCase()).includes(filterLower)
        );
    }
    ...
    return (
        <div className="w-full">
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-12">
                ...
                {filtered.length === 0 && (
                    <div className="col-span-full text-center text-zinc-500 dark:text-zinc-400">No posts found.</div>
                )}
            </section>
        </div>
    );
